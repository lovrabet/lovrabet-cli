...
| 查看云端 Skill 列表 | `lovrabet skill list [--scope user|personal|company|public|all]` |
| 查看本地 CLI 管理的 Skill | `lovrabet skill list --local [--scope user|personal|company|public|all]` |
| 拉取当前应用个人级和公司级 Skill | `lovrabet skill pull` |
...
`lovrabet skill create/validate/list/pull/push` 是 CLI 的 Skill 工作流：create 只生成本地自包含草稿，validate 检查 `SKILL.md` 与 frontmatter 必要字段，list 查看 SkillHub-backed 云端列表或本地 CLI 管理的 cache/链接，pull 把当前应用 personal/company 业务 Skill 拉到本地，push 只创建或更新 personal Skill。Builtin Skill 由 sandbox 镜像内置提供，不通过远端 `skill list/pull` 管理。`skill list --local` 只读本地元数据，不下载、不物化、不清理。`skill push` 上传前也只检查 `SKILL.md` 和 frontmatter 必要字段。frontmatter `description` 应写模型触发语义，说明何时使用、不要使用的边界和关键词。Skill 正文、章节、references、占位符、输出协议、明文凭证和外部链接不参与 CLI validate。RuntimeAgent 的 `skill_load`、`skill_update` 等原生工具不属于 CLI 命令，不要把它们写进 `lovrabet` 命令步骤。
...
| write | data create, data batchCreate, data update, personal-bff create/update, artifact create/update, kb create/update, service import/export/remove, skill create/pull/push, workspace init/use, app init/use/import, config set/delete | dry-run 预览 |
