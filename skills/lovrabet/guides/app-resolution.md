...
应用决议的目标只有一个：确定这次操作应该落在哪个 app 上。不要把它和“刷新配置”“同步应用列表”混在一起。
...
5. 命中后再进入 `dataset detail`、`data *`、`sql *`、`bff *`
