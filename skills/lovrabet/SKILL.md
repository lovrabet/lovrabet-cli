...
- **`app use` / `app import`**：只操作本地用户意图配置（`defaultApp` / 顶层 `appcode`）。如果前一步用 `app list --env daily` 查到应用，`app use` 也要带同一个 `--env daily`，避免按默认环境查缓存。`app use` 不允许把未发布应用设为默认候选。详见 [应用管理](references/lovrabet-app.md)。
...
| **service** | `validate` | 校验 Service Tree JSON manifest | read | 本地 |
| **service** | `import` | 导入 Service Tree manifest 到 `~/.lovrabet/service.json` | write | 本地 |
...
| 查看云端 Skill 列表 | `lovrabet skill list [--scope user|personal|company|builtin|all]` |
| 查看本地 CLI 管理的 Skill | `lovrabet skill list --local [--scope user|personal|company|builtin|all]` |
...
`lovrabet skill create/validate/list/pull/push` 是 CLI 的 Skill 工作流：create 只生成本地自包含草稿，validate 检查 `SKILL.md` 与 frontmatter 必要字段，list 查看云端列表或本地 CLI 管理的 cache/链接，pull 把当前应用 personal/company Skill 拉到本地，push 只创建或更新 personal Skill。`skill list --local` 只读本地元数据，不下载、不物化、不清理。`skill push` 上传前也只检查 `SKILL.md` 和 frontmatter 必要字段。frontmatter `description` 应写模型触发语义，说明何时使用、不要使用的边界和关键词。Skill 正文、章节、references、占位符、输出协议、明文凭证和外部链接不参与 CLI validate。RuntimeAgent 的 `skill_load`、`skill_update` 等原生工具不属于 CLI 命令，不要把它们写进 `lovrabet` 命令步骤。
...
`lovrabet service validate/import/export/list/detail/remove` 管理本地 Service Tree registry。首期 JSON 文件通常由 Git 管理，导入后写入 `~/.lovrabet/service.json`。动态业务命令形态为 `lovrabet <service> [...resourcePath] <action> [flags]`，例如 `lovrabet crm customer list --status active`，由 manifest 映射到底层 `data` / `sql` / `bff` 执行。导入后的服务会出现在 `lovrabet --help`、`lovrabet schema` 和 `lovrabet doctor` 的发现/诊断输出中。动态命令仍走统一认证、appcode 决议、已发布应用访问限制、risk、dry-run、`--format` 和 `--jq` 管道。详见 [lovrabet-service-tree.md](references/lovrabet-service-tree.md)。
...
| 等级 | 命令 | 保护 |
|------|------|------|
| read | api-doc list/detail, dataset list/detail/sdk-doc, data filter/getOne/aggregate, sql detail/exec, bff detail/exec, personal-bff list/detail, artifact list/detail, kb list/detail/search, service validate/list/detail, skill validate/list, app list, config list/get, logs | 无 |
| write | data create, data batchCreate, data update, personal-bff create/update, artifact create/update, kb create/update, service import/export/remove, skill create/pull/push, app use, config set/delete, app import | dry-run 预览 |
