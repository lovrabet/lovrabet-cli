...
name: lovrabet
version: 2.1.7
description: "Lovrabet 运行态 CLI — 面向业务场景的 AI 操作套件，通过 lovrabet 命令管理应用目录、Service Tree 业务命令、API 文档发现、数据集查询、Instant API 数据操作、SQL/BFF、personal BFF、Artifact、Skill、知识库与运行态 app-config key 状态检查。触发词：云图、lovrabet、lovrabet-cli、service tree、业务服务树、api-doc、dataset、data filter、artifact、personal-bff、kb、skill、sql exec、bff exec、app-config、accessKey、compress、jq。"
metadata:
  requires:
    bins: ["lovrabet"]
    node: ">=20"
...
| 等级 | 命令 | 保护 |
|------|------|------|
| read | api-doc list/detail, dataset list/detail/sdk-doc, data filter/getOne/aggregate, sql detail/exec, bff detail/exec, app-config get, personal-bff list/detail, artifact list/detail, kb list/detail/search, service validate/list/detail, skill validate/list, app list, config list/get, logs | 无 |
